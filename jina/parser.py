...
def set_base_parser():
    ...
    url_str = '\n'.join(
        f'{v[0]} {k:10.10}\t{colored(v[1], "cyan", attrs=["underline"])}'
...
def set_hub_new_parser(parser=None):
    ...
    parser.add_argument(
        '--template',
        type=str,
        default='https://github.com/jina-ai/cookiecutter-jina-hub.git',
...
def set_hub_list_parser(parser=None):
    ...
    parser.add_argument('--name', type=str, help='name of hub image')
    parser.add_argument('--kind', type=str, help='kind of hub image')
    parser.add_argument(
        '--keywords', type=str, nargs='+', help='keywords for searching'
    )
    parser.add_argument(
        '--type',
        type=str,
...
def set_hw_parser(parser=None):
    ...
    gp.add_argument(
        ...
        default=resource_filename(
            'jina', '/'.join(('resources', 'logserver.default.yml'))
        ),
    ...
    gp.add_argument(
        '--shards', type=int, default=2, help='number of shards when index and query'
    )
    gp.add_argument(
        '--parallel',
        type=int,
    ...
    gp.add_argument(
        ...
        type=str,
        default=resource_filename(
            'jina', '/'.join(('resources', 'helloworld.flow.index.yml'))
        ),
    ...
    gp.add_argument(
        '--index-batch-size', type=int, default=1024, help='the batch size in indexing'
    )
    ...
    gp.add_argument(
        ...
        type=str,
        default=resource_filename(
            'jina', '/'.join(('resources', 'helloworld.flow.query.yml'))
        ),
    ...
    gp.add_argument(
        '--query-batch-size', type=int, default=32, help='the batch size in searching'
    )
    gp.add_argument(
        '--num-query', type=int, default=128, help='number of queries to visualize'
    )
    gp.add_argument(
...
def set_flow_parser(parser=None):
    ...
    gp.add_argument(
        '--optimize-level',
        type=FlowOptimizeLevel.from_string,
        default=FlowOptimizeLevel.NONE,
    ...
    gp.add_argument(
        '--output-type',
        type=FlowOutputType.from_string,
        choices=list(FlowOutputType),
        default=FlowOutputType.SHELL_PROC,
...
def set_pea_parser(parser=None):
    ...
    gp2.add_argument(
        '--port-ctrl',
        type=int,
        default=os.environ.get('JINA_CONTROL_PORT', random_port()),
    ...
    gp5.add_argument(
        '--skip-on-error',
        type=OnErrorSkip.from_string,
        choices=list(OnErrorSkip),
    ...
    gp8.add_argument(
        '--ssh-password', type=str, default=None, help='ssh password to the ssh server.'
    )
...
def set_ping_parser(parser=None):
    ...
    parser.add_argument(
        'host', type=str, help='host address of the target pod/pea, e.g. 0.0.0.0'
    )
...
def set_client_cli_parser(parser=None):
    ...
    gp1.add_argument(
        '--mode',
        choices=list(ClientMode),
        type=ClientMode.from_string,
        # required=True,
        help='the mode of the client and the server',
    )
    gp1.add_argument(
        '--top-k', type=int, help='top_k results returned in the search mode'
    )
    gp1.add_argument(
        '--mime-type',
        type=str,
...
def get_main_parser():
    ...
    set_flow_parser(
        sp.add_parser(
            ...
            help='start a flow from a YAML file',
            formatter_class=_chf,
        )
    )
    set_gateway_parser(
                ...
                if isinstance(action, argparse._StoreTrueAction):
                    ...
                    help += colored(
                        ...
                        % (
                            'enabled'
                            if action.default
                            else 'disabled, use "--%s" to enable it' % action.dest
                        ),
                        attrs=['dark'],
                    )
                elif action.choices:
    ...
    def _metavar_formatter(self, action, default_metavar):
        if action.metavar is not None:
            ...
            if len(action.choices) > 4:
                ...
                result = '{%s ... %d more choices}' % (
                    choice_strs,
                    len(action.choices) - 4,
                )
            else:
