...
class MIMEDriver(BaseConvertDriver):
    ...
    def convert(self, d):
        ...
        if m_type:
            ...
            self.logger.warning(
                f'can not determine the MIME type, set to default {self.default_mime}'
            )
...
class NdArray2PngURI(BaseConvertDriver):
    ...
    def png_convertor_1d(self, arr: np.array):
        ...
        png_bytes = b''.join(
            [
                ...
                png_pack(
                    b'IHDR',
                    struct.pack('!2I5B', self.width, self.height, 8, 6, 0, 0, 0),
                ),
    ...
    def png_convertor(self, arr: np.array):
        ...
        if len(arr.shape) == 1:
            ...
            im = im.resize(
                (self.width, self.height), getattr(Image, self.resize_method)
            )
...
class Blob2PngURI(NdArray2PngURI):
    ...
    def __init__(
        self, target='uri', width: int = 28, height: int = 28, *args, **kwargs
    ):
...
class URI2DataURI(URI2Buffer):
    ...
    def make_datauri(self, mimetype, data, binary=True):
        ...
        if self.base64:
            ...
            if binary:
                encoded_data = (
                    encode64(data).decode(self.charset).replace('\n', '').strip()
                )
            else:
