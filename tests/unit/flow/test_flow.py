...
def test_flow_yaml_dump():
    f = Flow(
        logserver_config=os.path.join(cur_dir, '../yaml/test-server-config.yml'),
...
def test_flow_log_server():
    ...
    with f:
        ...
        # Ready endpoint
        a = requests.get(JINA_GLOBAL.logserver.address + '/status/ready', timeout=5)
        assert a.status_code == 200

        # YAML endpoint
        a = requests.get(JINA_GLOBAL.logserver.address + '/data/yaml', timeout=5)
        assert a.text.startswith('!Flow')
        assert a.status_code == 200

        # Pod endpoint
        a = requests.get(JINA_GLOBAL.logserver.address + '/data/api/pod', timeout=5)
        assert 'pod' in a.json()
        assert a.status_code == 200

        # Shutdown endpoint
        a = requests.get(JINA_GLOBAL.logserver.address + '/action/shutdown', timeout=5)
        assert a.status_code == 200

        # Check ready endpoint after shutdown, check if server stopped
        with pytest.raises(requests.exceptions.ConnectionError):
            requests.get(JINA_GLOBAL.logserver.address + '/status/ready', timeout=5)
...
def test_refactor_num_part_2():
    ...
    f = Flow().add(name='r1', uses='_logforward', needs='gateway', parallel=3)
...
def test_flow_with_publish_driver():
    ...
    with f:
        f.index_lines(
            lines=['text_1', 'text_2'], output_fn=validate, callback_on_body=True
        )
...
def test_flow_with_modalitys_simple():
    ...
    flow = (
        Flow()
        .add(name='chunk_seg', parallel=3, uses='_pass')
        .add(
            name='encoder12',
            parallel=2,
