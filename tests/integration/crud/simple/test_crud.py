...
def test_delete_vector(config, mocker, flow_file, has_content, compound):
    ...
    with Flow.load_config(flow_file) as search_flow:
        search_flow.search(
            input_fn=random_docs(0, NUMBER_OF_SEARCHES),
...
def test_delete_kv(config, mocker, as_string):
    ...
    with Flow.load_config(flow_file) as search_flow:
        search_flow.search(
            input_fn=chain(random_docs(2, 5), random_docs(100, 120)),
    ...
    with Flow.load_config(flow_file) as search_flow:
        search_flow.search(
            input_fn=random_docs(2, 4), on_done=validate_result_factory(1)
        )
    mock.assert_called_once()
...
def test_update_vector(config, mocker, flow_file, compound):
    ...
    with Flow.load_config(flow_file) as search_flow:
        ...
        search_flow.search(
            input_fn=search_docs, on_done=validate_result_factory(has_changed=False)
        )
    mock.assert_called_once()
...
def test_update_kv(config, mocker):
    ...
    with Flow.load_config(flow_file) as search_flow:
        search_docs = list(random_docs(0, NUMBER_OF_SEARCHES))
        search_flow.search(input_fn=search_docs, on_done=validate_results)
    mock.assert_called_once()
    ...
    with Flow.load_config(flow_file) as search_flow:
        search_flow.search(
            input_fn=random_docs(0, NUMBER_OF_SEARCHES), on_done=validate_results
        )
    mock.assert_called_once()
